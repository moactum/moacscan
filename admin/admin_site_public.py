from django.contrib import admin
from jsonstore.models import *
from moac.models import *
from django.contrib.admin.actions import delete_selected
from decimal import Decimal
import csv
from django.http import HttpResponse
from django.utils.translation import gettext as _

# Register your models here.
class JsonMoacBlockAdmin(admin.ModelAdmin):
	list_display = ('id','data')
	def has_change_permission(self,request,obj=None):
		if obj:
			return False
		return True
	def has_module_permission(self,request):
		return True

class AddressAdmin(admin.ModelAdmin):
	list_display = ('display','balance','is_contract','created','changed','address')
	readonly_fields = ('address','balance','is_contract','created','changed', 'display','code')
	list_filter = ('is_contract','app_only')
	search_fields = ('=address','display')
	ordering = ('-changed','-balance')
	date_hierarchy = 'changed'

	def has_change_permission(self,request,obj=None):
		return True
	def has_module_permission(self,request):
		return True
	def save_model(self, request, obj, form, change):
		pass
	def delete_model(request, obj):
		pass

class TokenAdmin(admin.ModelAdmin):
	list_display = ('symbol','token_type','decimals','address','name','total_supply')
	list_filter = ('token_type',)
	ordering = ('-created',)
	date_hierarchy = 'created'

	def has_change_permission(self,request,obj=None):
		return True
	def has_module_permission(self,request):
		return True
	def save_model(self, request, obj, form, change):
		pass
	def delete_model(request, obj):
		pass

class UncleAdmin(admin.ModelAdmin):
	list_display = ('block','number','miner','hash')
	readonly_fields = ('number','miner','block','hash')
	search_fields = ('=hash','=number','miner__address')
	ordering = ('-block__id','-number')
	date_hierarchy = 'block__date'

	def has_change_permission(self,request,obj=None):
		return True
	def has_module_permission(self,request):
		return True
	def save_model(self, request, obj, form, change):
		pass
	def delete_model(request, obj):
		pass

class BlockAdmin(admin.ModelAdmin):
	list_display = ('id','num_txs','tps','duration','date','miner','hash')
	readonly_fields = ('id','num_txs','tps','duration','date','timestamp','miner','hash','difficulty','nonce')
	search_fields = ('=hash','=id','miner__address')
	ordering = ('-id',)
	date_hierarchy = 'date'

	def has_change_permission(self,request,obj=None):
		return True
	def has_module_permission(self,request):
		return True
	def save_model(self, request, obj, form, change):
		pass
	def delete_model(request, obj):
		pass

class TransactionAdmin(admin.ModelAdmin):
	list_display = ('block','tx_from','nonce','tx_to','value_moac','index','hash')
	readonly_fields = ('block','tx_from','tx_to','value','nonce','value_moac','index','hash')
	search_fields = ('=hash','=block__id','=block__hash','tx_from__address','tx_to__address','tx_from__display','tx_to__display')
	ordering = ('-block__id','-index')
	date_hierarchy = 'block__date'

	def has_change_permission(self,request,obj=None):
		return True
	def has_module_permission(self,request):
		return True
	def save_model(self, request, obj, form, change):
		pass
	def delete_model(request, obj):
		pass

class AdminSite(admin.sites.AdminSite):
	site_header = _('MOACGURU')
	site_title  = _('MOACGURU')
	index_title = _('MOACGURU')

	def has_permission(self,request):
		return True

my_admin_site_public = AdminSite(name='public')
my_admin_site_public.disable_action('delete_selected')
my_admin_site_public.register(Address,AddressAdmin)
my_admin_site_public.register(Token,TokenAdmin)
my_admin_site_public.register(Uncle,UncleAdmin)
my_admin_site_public.register(Block,BlockAdmin)
my_admin_site_public.register(Transaction,TransactionAdmin)
#my_admin_site_public.register(JsonMoacLedger,JsonMoacLedgerAdmin)
